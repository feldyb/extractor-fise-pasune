...
function bindUI(){
  ...
  document.getElementById('loadFromTemplateBtn').addEventListener('click', loadFisaFromTemplate);
}
...
async function loadFisaFromTemplate(){
  if (!templateArrayBuffer){
    toast('Încarcă mai întâi șablonul Word (.docx) mai sus.');
    return;
  }
  toast('Se citesc fișele din șablon...');
  try {
    await DocxEditor.load(templateArrayBuffer.slice(0));
  } catch(err){
    toast('Eroare la citirea șablonului: ' + err.message);
    return;
  }
  const dicts = {
    plants: CODEBOOK.plants, soils: CODEBOOK.soils, shrubs: CODEBOOK.shrubs,
    relief: CODEBOOK.relief, config_teren: CODEBOOK.config_teren
  };
  let extracted;
  try {
    extracted = DocxEditor.extractAllFisas(dicts);
  } catch(err){
    toast('Eroare la extragere: ' + err.message);
    return;
  }
  const existingUA = new Set(fise.map(f=>String(f.u_a).trim().toUpperCase()));
  let added = 0, skipped = 0;
  extracted.forEach(raw=>{
    const uaKey = String(raw.u_a||'').trim().toUpperCase();
    if (!uaKey || existingUA.has(uaKey)){ skipped++; return; }
    const fisa = normalizeFisa(raw);
    fisa.id = 'f'+Date.now()+Math.random().toString(36).slice(2,7)+added;
    fisa.thumb = null;
    fise.push(fisa);
    existingUA.add(uaKey);
    added++;
  });
  saveFise();
  renderFiseList();
  toast(`Aduse ${added} fișe noi din șablon${skipped?` (${skipped} deja existau în aplicație)`:''}.`);
  switchTab('review');
}
...
// Parsează un string liber -> {cod: pct|''} pentru lucrările recunoscute
// Recunoaște atât formatul nou ("Denumire lucrare pe 20%") cât și cel vechi ("625 pe 20%")
function parseLucrString(text){
  const result = {};
  if (!text) return result;
  for (const [cod, denumire] of Object.entries(CODEBOOK.lucrari)){
    const escDen = denumire.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
    const reDen = new RegExp(escDen + '(?:\\s*pe\\s*(\\d+(?:[.,]\\d+)?%))?', 'i');
    const mDen = text.match(reDen);
    if (mDen){ result[cod] = mDen[1] || ''; continue; }
    const escCod = cod.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
    const reCod = new RegExp('\\b' + escCod + '\\b(?:\\s*pe\\s*(\\d+(?:[.,]\\d+)?%))?', 'i');
    const mCod = text.match(reCod);
    if (mCod) result[cod] = mCod[1] || '';
  }
  return result;
}
...
function rebuildLucrString(container){
  const parts = [];
  Object.entries(CODEBOOK.lucrari).forEach(([cod, denumire])=>{
    ...
    if (cb && cb.checked){
      ...
      parts.push(pct ? `${denumire} pe ${pct}` : denumire);
    }
  });
  return parts.join('; ');
}
...
function renderLucrChecklist(host, f){
  ...
  host.innerHTML = Object.entries(CODEBOOK.lucrari).map(([cod, denumire])=>{
    ...
    return `<div class="checklist">
      ...
      <input type="text" data-lucr-pct="${cod}" value="${esc(pctVal)}" placeholder="%" class="pctfield" style="width:52px;flex:none;padding:4px 6px;">
    </div>`;
  }).join('');
...
// Dacă utilizatorul a scris doar cifre (ex. "50"), adaugă automat simbolul %.
// Nu atinge câmpurile deja goale sau care au deja alt format (ex. "40-50%", "-", text liber).
...
function bindPercentAutoSuffix(container){
  container.querySelectorAll('input.pctfield').forEach(inp=>{
    inp.addEventListener('blur', ()=>{
      const fixed = ensurePercent(inp.value);
      if (fixed !== inp.value){
        inp.value = fixed;
...
function renderFisaBody(container, f){
  ...
  container.innerHTML = `
    ...
    <div class="field-grid">
      ...
      ${fld(f,'alt','Altit.')}
      <div><label>Unit. sol</label>
        <input type="text" data-field="unit_sol" value="${esc(f.unit_sol)}">
        <span class="hint code-hint" data-hintfor="unit_sol" style="display:block;margin-top:2px;">${esc(soilName(f.unit_sol)||'')}</span>
      </div>
    </div>
    ...
    <div class="field-grid">
      ${fld(f,'tip_pajiste','Tip pajiște')} ${fld(f,'acoperire_ierbacee','Acoperire ierbacee','pctfield')}
    </div>
    ...
    <div class="field-grid">
      ${fld(f,'val_past','Val. past.')} ${fld(f,'gr_acop','Gr. acop.','pctfield')}
      ${fld(f,'rasp','Răsp.')}
    </div>
  ...
  const HINT_FN = { unit_sol: (v)=> soilName(v) || '' };
  container.querySelectorAll('input[data-field], textarea[data-field]').forEach(inp=>{
    inp.addEventListener('input', ()=>{
      ...
      if (hintEl){
        const fn = HINT_FN[inp.dataset.field] || annotateCodes;
        hintEl.textContent = fn(inp.value);
      }
      saveFise();
    });
  });
  ...
  bindPercentAutoSuffix(container);
...
function fld(f, key, label, cls){
  return `<div><label>${label}</label><input type="text" data-field="${key}" value="${esc(f[key])}" class="${cls||''}"></div>`;
}
...
function speciesBlock(f, key, label, opts={}){
  ...
  return `
    <div class="section-title">${label} ${showTotal ? `<input type="text" style="width:60px;display:inline-block;margin-left:6px;"
       data-total="${key}" value="${esc(f[totalKey])}" class="pctfield">` : ''}</div>
...
function speciesRowHtml(key, i, s, lookup){
  ...
  return `<div class="sp-row" data-idx="${i}">
    ...
    <input class="pct pctfield" type="text" data-sp="pct" value="${esc(s.pct)}" placeholder="%">
