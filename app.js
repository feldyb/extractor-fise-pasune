...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('modelSelect').value = localStorage.getItem(LS_MODEL) || 'gemini-3.5-flash';
...
async function processAllPhotos(){
  ...
  const model = localStorage.getItem(LS_MODEL) || 'gemini-3.5-flash';
  if (!key){
  ...
  for (const p of todo){
    ...
    try {
      const data = await callGeminiWithRetry(key, model, p.base64, p.mimeType, (msg)=>{ hint.textContent = msg; });
    ...
    if (p !== todo[todo.length-1]){
      const pacingSec = parseFloat(localStorage.getItem(LS_PACING) || '7') || 7;
...
async function callGeminiWithRetry(key, model, base64, mimeType, onStatus){
  const maxRetries = 4;
  let lastErr;
  for (let attempt=0; attempt<=maxRetries; attempt++){
    try {
      return await callGemini(key, model, base64, mimeType);
    } catch(err){
      lastErr = err;
      ...
      for (let s=waitSec; s>0; s--){
        onStatus && onStatus(`Limită de cereri Gemini atinsă — reîncerc în ${s}s (încercarea ${attempt+2}/${maxRetries+1})...`);
        await sleep(1000);
      }
    }
  }
...
function renderFisaBody(container, f){
  ...
  container.innerHTML = `
    ...
    <p class="hint code-hint" data-hintfor="lucr_propuse">${esc(annotateCodes(f.lucr_propuse))}</p>
    <div class="btnbar">
